...
void main()
{
	...
	position = getPos(vec2(gl_in[0].gl_Position.z,gl_in[1].gl_Position.x));
	float begin = gl_in[0].gl_Position.x-gl_in[1].gl_Position.y;
	if (showYear>begin && showYear<gl_in[0].gl_Position.y) {
		float size = popSize;
		if ((showYear-begin)<1.0){
			boost = 1.0 - showYear +begin;
			size *= (1.0+boost);
		}
		drawFlatSprite( vec2(gl_in[0].gl_Position.z,gl_in[1].gl_Position.x), size, 0.0);
	}
}
